...
module.exports.transform = (exp) => {
  log.info('Transform called with:', exp);
  ...
  function handlePassthrough(message) {
    log.info('handlePassthrough called with:', message);
  ...
  log.debug('Evaluating expression="%s" on body=%j', stringifiedExpression, exp);
  const result = compiledExpression.evaluate(exp);
  log.debug('Evaluation completed, result=%j', result);
  let counter = 0;
  if (result === undefined) {
